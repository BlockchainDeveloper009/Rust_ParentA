...
To find the **second occurrence** of a substring or find a substring from the **end of the string** in Rust, here are some approaches and code examples with explanations:

***

### 1. Find Second Occurrence of a Substring

Rust's built-in `.find()` method only finds the first occurrence. To find the second, do a **two-step search**:
...
fn main() {
    ...
    if let Some(first_pos) = text.find(pattern) {
        // Search again starting just after the first match
        if let Some(second_pos) = text[first_pos + pattern.len()..].find(pattern) {
            println!("Second occurrence index: {}", first_pos + pattern.len() + second_pos);
        } else {
            println!("Second occurrence not found");
        }
    } else {
...
### 2. Find Substring from the End of a String (Last Occurrence)

Use `.rfind()` which finds the **last occurrence**:

```rust
fn main() {
    let text = "look for the last 'the' word in the string";
    let pattern = "the";

    if let Some(idx) = text.rfind(pattern) {
        println!("Last occurrence index: {}", idx);
    } else {
        println!("Pattern not found");
    }
}
...
- `.find(substring)` returns the index of the first match.
- To find subsequent matches, search starting **just after the previous match** by slicing the string.
- `.rfind(substring)` returns the index of the **last match**.
- Indexing carefully with offsets is important because `.find()` returns an index relative to the sliced string.

***

Would you like me to help with a reusable function that finds the nth occurrence of a substring or any other variations?

[1](https://www.youtube.com/watch?v=1JxSMRSWzCY)
[2](https://www.oreilly.com/library/view/c-cookbook/0596007612/ch04s11.html)
[3](https://stackoverflow.com/questions/67635564/split-string-at-nth-occurrence-of-a-character-in-rust)
[4](https://www.reddit.com/r/csharp/comments/tw1apq/is_there_a_reason_you_can_only_find_the_first_and/)
[5](https://users.rust-lang.org/t/how-to-find-a-substring-starting-at-a-specific-index/8299)
[6](https://users.rust-lang.org/t/split-a-str-until-the-nth-occurrence-and-get-the-rest/51257)
[7](https://users.rust-lang.org/t/how-to-get-a-substring-of-a-string/1351)
[8](https://users.rust-lang.org/t/split-a-str-until-the-nth-occurrence-and-get-the-rest/51257/3)